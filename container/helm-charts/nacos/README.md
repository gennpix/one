...
| `image`                    | `nacos` image, tag.             | `bitnami/nginx` `latest`               |
...
```
$ helm install --name my-release -f values.yaml nacos
```
...
The [nacos image](https://) stores the data and configurations at the `/data` path of the container.
